...
$con->t->assign('h1','セールスパートナーのお問い合わせ');
$con->readyPostCsrf();

//form情報アサイン
require_once('partner/inquiry/form.php');
...
if ( $con->isPost ){
    if($_POST['operation'] == 'check'){
        require_once('partner/inquiry/check.php');
        ...
        if($bl){
            //お問い合わせ///////////////////////////
            require_once('partner/inquiry/handle.php');
            ...
            //for company
            $mailManager->sendPartnerInquiry();

            //for iluna
            $mailManager->sendPartnerInquiry(TRUE);

            $con->safeExitRedirect('/partner/inquiry/finish',TRUE);
        }else{
            $con->safeExitRedirect('/partner/inquiry/input',TRUE);
        }
...
$con->append('partner/inquiry/'.$page);
