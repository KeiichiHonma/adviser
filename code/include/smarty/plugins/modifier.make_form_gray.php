...
function smarty_modifier_make_form_gray($form_name,$form_setting,$error,$colspan = SMARTY_BOOL_OFF,$isConfirm = SMARTY_BOOL_OFF)
{
    ...
    $html = '';
    $html .= "<tr>\n";
    $html .= '<td class="form_ttl"><table cellspacing="0" cellpadding="5"><tr><td lang="ja">';
    ...
    $html .= "</td></tr></table></td>\n";
    $html .= '<td lang="ja" class="form_data">';
    ...
    for ($i=0;$i<$count;$i++){
        ...
        case 'text':
            if($isConfirm === SMARTY_BOOL_ON){
                ...
                //特殊処理
                if($setting[$i]['name'] == 'mail'){
                    $data = '<input lang="ja" type="text" name="'.$setting[$i]['name'].'" value="'.$escape_post.'" class="'.$setting[$i]['class'].'"'.$maxlength.' /><br class="clear" />';
                }elseif($setting[$i]['name'] == 'mail_m'){
                    $data = '<input lang="ja" type="text" name="'.$setting[$i]['name'].'" value="'.$escape_post.'" class="'.$setting[$i]['class'].'"'.$maxlength.' /><p class="p_fl">&nbsp;＠&nbsp;</p>';
                }else{
                    $data = '<input lang="ja" type="text" name="'.$setting[$i]['name'].'" value="'.$escape_post.'" class="'.$setting[$i]['class'].'"'.$maxlength.' />';
                }
        ...
        case 'password':
            if($isConfirm === SMARTY_BOOL_ON){
                ...
                $data = '<input lang="ja" type="password" name="'.$setting[$i]['name'].'" value="" class="'.$setting[$i]['class'].'"'.$maxlength.' />';
            }
            $html .= $setting[$i]['front'].$data.$setting[$i]['back'];
            break;
        ...
        case 'textarea':
            if($isConfirm === SMARTY_BOOL_ON){
                ...
                $data = '<textarea lang="ja" name="'.$setting[$i]['name'].'" class="'.$setting[$i]['class'].'" />'.$escape_post.'</textarea>';
            }
            $html .= $setting[$i]['front'].$data.$setting[$i]['back'];
            break;
                ...
                if($con->pagepath != 'entry/input' && (strcasecmp($setting[$i]['name'],'magazine_mail') == 0 || strcasecmp($setting[$i]['name'],'notification_mail') == 0)){
                    foreach ($setting[$i]['value'] as $key => $value){
                        ...
                        if(strlen($_POST['mail']) == 0 && strcasecmp($key,RECEIVE_PC) == 0){
                            $labe_id = '';
                            $data .= '<input lang="ja" disabled="true" type="radio" id="'.$labe_id.'" name="'.$setting[$i]['name'].'" value="'.$key.'" class="'.$setting[$i]['class'].'" '.$checked.' /><label for="'.$labe_id.'">'.$value.'</label>&nbsp;';
                            $data .= '(<span lang="ja" class="error_alert">！</span><a href="'.CAURL.'/mypage/member/edit/mail/input">パソコンのメールアドレスを設定</a>する必要があります。)';
                            $head_flag = TRUE;
                        //携帯で受信
                        }elseif(strlen($_POST['mail_m']) == 0 && strcasecmp($key,RECEIVE_MOBILE) == 0){
                            $labe_id = '';
                            $data .= '<br /><input lang="ja" disabled="true" type="radio" id="'.$labe_id.'" name="'.$setting[$i]['name'].'" value="'.$key.'" class="'.$setting[$i]['class'].'" '.$checked.' /><label for="'.$labe_id.'">'.$value.'</label>&nbsp;';
                            $data .= '(<span lang="ja" class="error_alert">！</span><a href="'.CAURL.'/mypage/member/edit/mail/input">携帯のメールアドレスを設定</a>する必要があります。)';
                            $foot_flag = TRUE;
                        //パソコン,携帯で受信
                        }elseif((strlen($_POST['mail']) == 0 || strlen($_POST['mail_m']) == 0) && strcasecmp($key,RECEIVE_ALL) == 0){
                            $labe_id = '';
                            $data .= '<br /><input lang="ja" disabled="true" type="radio" id="'.$labe_id.'" name="'.$setting[$i]['name'].'" value="'.$key.'" class="'.$setting[$i]['class'].'" '.$checked.' /><label for="'.$labe_id.'">'.$value.'</label>&nbsp;';
                            $data .= '(<span lang="ja" class="error_alert">！</span><a href="'.CAURL.'/mypage/member/edit/mail/input">パソコン、携帯両方のメールアドレスを設定</a>する必要があります。)';
                        }else{
                            ...
                            $data .= !$head_flag ?  '' : '<br />';
                            $data .= !$foot_flag ?  '' : '<br />';
                            $data .= '<input lang="ja" type="radio" id="'.$labe_id.'" name="'.$setting[$i]['name'].'" value="'.$key.'" class="'.$setting[$i]['class'].'" '.$checked.' /><label for="'.$labe_id.'">'.$value.'</label>&nbsp;';
                            $i2++;
                        }
                    }
                }else{
...
/*                    elseif((strlen($escape_post) == 0 || !isset($escape_post)) && $i2 == 0){
                    ...
                    $labe_id = $setting[$i]['name'].'_'.$i2;
                    $data .= '<input lang="ja" type="checkbox" id="'.$labe_id.'" name="'.$setting[$i]['name'].'[]" value="'.$key.'" class="'.$setting[$i]['class'].'" '.$checked.' /><label for="'.$labe_id.'">'.$value.'</label>&nbsp;';
                    $i2++;
                }
            }
            $html .= $setting[$i]['front'].$data.$setting[$i]['back'];
        ...
        case 'select'://セレクト：valueの数だけoptionが表示される.selectではないことに注意
            if($isConfirm === SMARTY_BOOL_ON){
                ...
                if($setting[$i]['name'] == 'carrier'){
                    $data .= '<select lang="ja" name="'.$setting[$i]['name'].'" class="mail_select_fl">'."\n";
            ...
            if($setting[$i]['name'] == 'carrier'){
                ...
                if($setting[$i]['front'] != '') $html .= '<span lang="ja" class="m_r10">'.$setting[$i]['front'].'</span>';//前テキストがある場合はマージンをとる
                $html .= $data;
                $html .= $setting[$i]['back']."\n";
        ...
        if($isConfirm === SMARTY_BOOL_ON){
            if(!is_array($escape_post)){
                $html .= '<input lang="ja" type="hidden" name="'.$setting[$i]['name'].'" value="'.$escape_post.'" />';//hidden
            }else{
                foreach ($escape_post as $key => $value){
                    $html .= '<input lang="ja" type="hidden" name="'.$setting[$i]['name'].'[]" value="'.$value.'" />';//hidden
                }
            }
